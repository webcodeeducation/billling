...
class Ledger{
  ...
  private $dbConnect = false;
    ...
    public function saveInvoice($data){
    	...
    	/*$updateSQL1 = 'update ' . $this->CustomerTable . ' set customer_name='"' . $name . "', customer_address='. $address  . ', customer_number='. $phone . ', customer_pan='. $pan . ' where customer_id='. $customer_id;*/
    	$updateSQL = 'update customers set customer_name="'.$name.'",customer_address="'.$address.'",customer_number="'.$phone.'",customer_pan="'.$pan.'" where customer_id='.$customer_id;
    	$result = mysqli_query($this->dbConnect, $updateSQL);
    	return true;
    }
